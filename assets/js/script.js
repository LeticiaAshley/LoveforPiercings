/* PRODUTOS - catálogo completo de joias disponíveis no site.
   Cada objeto representa um produto com metadados usados para
   renderização, busca, filtros e adição ao carrinho.
*/
...
/* Filtra produtos por tipo de perfuração.
   Usado para exibir apenas os produtos relevantes à página atual.
*/
...
/* Elementos do DOM compartilhados entre catálogo, filtro e carrinho */
...
/* Normaliza o caminho de imagem para garantir valores consistentes.
   Aceita caminhos já completos ou adiciona o prefixo padrão quando necessário.
*/
...
/* Normaliza texto para busca por termo sem acentuação e sem diferença entre maiúsculas/minúsculas. */
...
/* Renderiza a lista de produtos no catálogo. Cada produto gera um card com imagem,
   preço, descrição e botão de compra. */
...
/* Filtra o catálogo com base no texto de pesquisa e na ordenação selecionada.
   A filtragem é aplicada de forma não destrutiva usando uma cópia do array original.
*/
function filtrar(){
...
/* Atualiza a listagem e totais do carrinho na interface.
   Responsável por renderizar cada item do carrinho e recalcular valores.
*/
...
/* Controla a visibilidade do modal do carrinho.
   Abre e fecha o overlay que exibe os itens do carrinho.
*/
...
/* Registra os eventos de interação do usuário para pesquisa e ordenação.
   Cada evento dispara a filtragem em tempo real do catálogo.
*/
if(pesquisa){
...
/* Inicializa o catálogo com todos os produtos, antes de aplicar filtros de página. */
renderizar(produtos);
 
/* Integração com PayPal.
   Cria o botão de pagamento somente se a biblioteca PayPal estiver disponível.
*/
...
/* Detecta a página atual para aplicar o filtro de categoria correspondente.
   As páginas de produto exibem apenas a categoria correta ao carregar.
*/
